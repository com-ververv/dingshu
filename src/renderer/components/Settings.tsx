...
export function Settings({ onClose }: SettingsProps) {
  ...
  const handleSaveCredentials = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setCredentialResult({
          success: false,
          message: result.error?.message ?? '保存失败。',
        });
      }
    } finally {
  ...
  const handleStartLarkAuth = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setCredentialResult({
          success: false,
          message: result.error?.message ?? '启动飞书授权失败。',
        });
      }
    } finally {
      setAuthBusy(false);
  ...
  const handleCompleteLarkAuth = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setCredentialResult({
          success: false,
          message: result.error?.message ?? '完成飞书授权失败。',
        });
      }
    } finally {
      setAuthBusy(false);
  ...
  const handleMigrateToDocuments = async () => {
    if (!dbInfo?.isLegacyLocation) {
      setMigrationResult({
        success: false,
        message: 'Database is already in Documents folder.',
      });
      return;
    }
            ...
            {activeTab === 'ai' && (
              <section className="settings-section">
                ...
                <div className={`db-status ${secureStatus?.safeStorageAvailable ? 'success' : 'warning'}`}>
                  ...
                  <span>
                    {secureStatus?.safeStorageAvailable ? 'safeStorage 可用' : 'safeStorage 不可用，无法保存凭证'}
                  </span>
                </div>
                ...
                {credentialResult && (
                  <div className={`test-result ${credentialResult.success ? 'success' : 'error'}`}>
            ...
            {activeTab === 'lark' && (
              <section className="settings-section">
                ...
                <p className="section-description">
                  appId / appSecret 会加密保存；用户授权由 lark-cli 设备码流程完成。
                </p>
                <div className="credential-grid">
                  ...
                  {secureStatus?.larkAppIdConfigured && !secureStatus.larkAppIdHealthy ? (
                    <div className="credential-warning">
                      ...
                      {secureStatus.credentialErrors.larkAppId ? (
                        <small>{secureStatus.credentialErrors.larkAppId}</small>
                      ) : null}
                    </div>
                  ) : null}
                  <input
                ...
                <button
                  ...
                  disabled={
                    savingCredentials ||
                    !secureStatus?.safeStorageAvailable ||
                    (!larkAppId.trim() && !larkAppSecret.trim())
                  }
                >
            ...
            {activeTab === 'about' && (
              <section className="settings-section about-section">
                ...
                <p className="about-text">
                  Pexar Lark Agent 是桌面端飞书 AI
                  助手，可通过自然语言搜索文档、查询消息、创建文档和发送消息，并在写操作前提供应用内二次确认。
                </p>
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      ...
                      window.api.shell.openExternal(
                        `https://github.com/${__APP_CONFIG__.github.owner}/${__APP_CONFIG__.github.repo}`,
                      );
                    }}
                  >
                    GitHub Repository
